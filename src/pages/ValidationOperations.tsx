...
export default function ValidationOperations() {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            ...
            setSegments(segs);
            setOrtherOperations(
                ortherOps.filter(o => o.status === "pending")
            );
        ...
        try {
            ...
            if ("partner_id" in op) {
                ...
                const { operation, segments: updatedSegments } =
                    await operationWorkflowService.validateOperation(
                        op,
                        opSegments ?? []
                    );
                ...
                setSegments(prev =>
                    prev.map(
                        s => updatedSegments.find(u => u.id === s.id) ?? s
                    )
                );
                ...
                await otherOperationService.update(op.id, {
                    status: "validated"
                });

                setOrtherOperations(prev =>
    ...
    const handleCancel = async (
        ...
        try {
            ...
            if ("partner_id" in op) {
                ...
                setSegments(prev =>
                    prev.map(
                        s => opSegments.find(u => u.id === s.id) ?? s
                    )
                );
